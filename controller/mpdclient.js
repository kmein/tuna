...
function connect() {
  ...
  mpdClient.on("ready", function () {
    console.log(
      "MPD client ready and connected to " +
        mpdOptions.host +
        ":" +
        mpdOptions.port
    );
...
function retryConnect() {
  if (mpdStatus === Status.reconnecting) return;
  mpdClient = null;
...
function sendCommands(commands, callback) {
  try {
    if (mpdStatus !== Status.ready) callback("Not connected");
    ...
    if (Array.isArray(commands)) mpdClient.sendCommands(commands, cb);
    else mpdClient.sendCommand(commands, cb);
  } catch (error) {
    callback(error);
  }
}
...
function sendStatusRequest(callback) {
  sendCommands([cmd("currentsong", []), cmd("status", [])], function (
    err,
    msg
  ) {
    if (err) {
